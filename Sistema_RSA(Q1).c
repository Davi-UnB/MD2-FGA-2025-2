...
int criptografar(int mensagem, int expoente, int n, int numEuler, int seletor) {
    ...
    if (n == 1) printf("\nQueremos encontrar C tal que C congruente a %d^%d mod %d\n", mensagem, expoente, n);
    else printf("\nQueremos encontrar M tal que M congruente a %d^%d mod %d\n", mensagem, expoente, n);
...
int main(void) {
    ...
    if (primos[0] == primos[1]) {
        printf("\nERRO: Os fatores primos encontrados sao iguais (p = q = %d).\n", primos[0]);
        printf("O RSA exige primos distintos para funcionar corretamente.\n");
        return 1;
    }
    ...
    int n = p * q;
    if (n <= 36) {
        printf("ERRO: O programa sera abortado pois n = %d, o que impede uma criptografia valida que respeite os intervalos definidos.\n", n);
        return 1;
    }
    int z = (p - 1) * (q - 1);
    int E = 0;
